    ...
    */ 
	...
	for(i=0;i<loop;i++) buf_red[i]=ptrs.inpbuf[i];
		...
		if (tmp == 0) { 
			...
			printf("reading 128 x 128\n");
			ptrs.flag = tmp;
						inptr = fopen("r128.bin","rb");
			if (!inptr)
			{
				printf("Unle to open file!");
				return 1;
			...
			printf("reading 64 x 64\n");
			ptrs.flag = tmp;
						inptr = fopen("r64.bin","rb");
			if (!inptr)
			{
				printf("Unle to open file!");
				return 1;
			...
			printf("reading 32 x 32\n");
			ptrs.flag = tmp;
						inptr = fopen("r32.bin","rb");
			if (!inptr)
			{
				printf("Unle to open file!");
				return 1;
	...
	//fwrite(alt,sizeof( int)1024,outptr);
	fclose(outptr);
